...
int AddNewBranch1(struct ElementOfTree** trunk, char* key, char* str)                                     //Функция добавления нового элемента графа
{
	//struct ElementOfTree** Branch = *trunk;
...
void Printing2(struct ElementOfTree* trunk, char* k)                                                            //Функция для Вывода элементов графа на экран
{
	if (trunk == NULL)
		return;

	Printing2(trunk->Left,k);
	Printing2(trunk->Right,k);
	if (trunk->key)
	{
		
			int len = strlen(k);
			if (strncmp(k, trunk->key,len) == 0)
			printf("\n\nKey:%s\nString:%s", trunk->key, trunk->str);
...
void PrintNode(struct ElementOfTree* trunk, int offset)
{
	...
	PrintNode(trunk->Right, offset + 1);
	for (int i = 0; i < offset; i++)
		printf("  ");
	if (trunk->key)
		printf("%s\n", trunk->key);
	PrintNode(trunk->Left, offset + 1);
}
...
struct ElementOfTree* SearchNode( struct ElementOfTree* Branch, char* key)                        //Вспомогательная Функция для поиска конкретного элемента графа
{
	if (Branch == NULL)
		return NULL;
	if (Branch->key == key)
...
{
	srand(time(NULL));
	static const char ab[] = "0123456789qwertyuiopasdfghjklzxcvbnmABCDEFGHIGKLMNOPQRSTUVWXYZ"; //Alphabet&Digit
